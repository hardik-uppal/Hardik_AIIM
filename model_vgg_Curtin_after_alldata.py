...
#    
    ...
    weights_path =  ('D:/tutorial/rgb+depth+thermal/all_data/result_vgg_depth_alldata/weights-14.h5')
    train_model.load_weights(weights_path)
    
    final_layer = train_model.layers[-6].output
...
#    
    output_final = layers.Dense(n_class, activation='softmax', name='output_final')(dropout_2_final)

    
    
    new_train_model = models.Model(inputs_depth,  output_final)
    for layer in new_train_model.layers[:-6]:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', default='./CurtinFaces/result_vgg_depth_pre')
