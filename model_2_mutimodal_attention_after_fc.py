...
#    dropout_rgb = layers.Dropout(0.2)(fc6_rgb)
    ...
    ######## for Depth 
    flat_model = layers.Flatten(name='flatten')(merge_rgb_depth)
...
#    attention_probs = layers.Dense(512, activation='softmax', name='attention_vec')(dropout_2)
#    attention_mul = layers.multiply([dropout_2, attention_probs],name='attention_mul')
#    attention_features = cbam_block(dropout_2, att_type='Spatial')
    ...
    train_model.summary()
    for layer in train_model.layers[:-9]:
        layer.trainable = False
...
#    (x_train, y_train), (x_test, y_test) = data
    ...
    def val_generator(batch_size):
        train_datagen = ImageDataGenerator(rescale=1./255, shear_range=0.2, zoom_range=0.2, horizontal_flip=True)  
        generator_rgb = train_datagen.flow_from_directory(directory="D:/CurtinFaces_crop/RGB/test", target_size=(224, 224), color_mode="rgb",
                                                      batch_size=batch_size, class_mode="categorical", shuffle=True, seed=42)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', default='./CurtinFaces_Norm/result_multimodal_attention_after_fc')
